...
cp = torch.load('checkpoints/model_epoch_98.pt', map_location=device)
...
print('\n' + '='*90)
print('  TEST: Epoch 98/200 - Temperature 0.4, Top-K 35')
print('  (Lower temperature = more deterministic, Higher top-k = more diversity)')
print('='*90 + '\n')

with torch.no_grad():
    for i, q in enumerate(questions, 1):
        tokens = [vocab.bos_idx] + vocab.encode(q, add_special_tokens=False)
        out = model.generate(tokens, max_length=50, temperature=0.4, top_k=35, device=device)
...
print('='*90)
print(f'  Model: Epoch 98/200 | Params: 21.5M | Temp: 0.4 | Top-K: 35')
print('='*90 + '\n')
