...
### 性能与实测（2026-07-14）
- 受控 A/B（8000 行×1 epoch，DML fp32）：增强开 Val 7.22 vs 关 7.72；但 DML 训练吞吐一度降至约 2370 tok/s（关约 3270，慢约 28%）。**优化②落地后**（见下）增强训练吞吐回升至约 **3424 tok/s**（与 BASE 差距缩至约 8%）。解码开销可忽略（top-k/IGMCG 约 0.85–0.90x）。详见 `experiments/cmp_enh_base.txt`、`experiments/cmp_4way.txt`。
...
8000 行语料（merged.txt 前 8000 行，确定性，非随机）× 1 epoch、`configs/config_cmp_base.yaml`（四关）/ `config_cmp_enh.yaml`（四开）/ `config_cmp_alt.yaml`（四开 + `enhancement_off_prob: 0.5` 整体随机交替关增强）/ `config_cmp_sel.yaml`（四开 + `enhancement_schedule` 分段**选择性**交替：attn_temp 恒开，qk_norm/residual_gate 按 4 段掩码循环切换）、DML(fp32) 下对照：
