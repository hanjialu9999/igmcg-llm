...
### 性能与实测（2026-07-14）
- 受控 A/B（8000 行×1 epoch，DML fp32）：增强开 Val 7.22 vs 关 7.72；但 DML 训练吞吐一度降至约 2370 tok/s（关约 3270，慢约 28%）。**优化②落地后**（见下）增强训练吞吐回升至约 **3424 tok/s**（与 BASE 差距缩至约 8%）。解码开销可忽略（top-k/IGMCG 约 0.85–0.90x）。详见 `experiments/cmp_enh_base.txt`、`experiments/cmp_3way.txt`。
...
### 实测对比（2026-07-14，同数据同参数受控 BASE / ENH / ALT）

8000 行语料（merged.txt 前 8000 行，确定性，非随机）× 1 epoch、`configs/config_cmp_base.yaml`（四关）/ `config_cmp_enh.yaml`（四开）/ `config_cmp_alt.yaml`（四开 + `enhancement_off_prob: 0.5` 按批次交替关增强）、DML(fp32) 下对照：

- **训练**：BASE(关) **Val 7.8300 / ~3735 tok/s**；ENH(常开) **Val 7.1021 / ~3424 tok/s**；ALT(交替 off=0.5) **Val 7.3543 / ~3596 tok/s**。② 后“常开”开销仅比 BASE 慢约 8%；交替比常开再快约 +5%，但 Val Loss 回升到 7.35，**介于 BASE 与 ENH 之间**，属速度/质量折中（非免费提速）。
- **生成（推理时 ALT 同常开）**：top-k BASE 40.1 / ENH 35.4 / ALT 36.2；IGMCG BASE 12.5 / ENH 10.8 / ALT 10.7 tok/s（增强版约 0.85–0.90x）。
- **文本**：8000 行/1 epoch 仍偏词块破碎；增强版输出略更连贯，与更低的 val loss 一致。
