...
### 性能与实测（2026-07-14）
- 受控 A/B（8000 行×1 epoch，DML fp32）：增强开 Val 7.22 vs 关 7.72；但 DML 训练吞吐一度降至约 2370 tok/s（关约 3270，慢约 28%）。**优化②落地后**（见下）增强训练吞吐回升至约 **3424 tok/s**（与 BASE 差距缩至约 8%）。解码开销可忽略（top-k/IGMCG 约 0.85–0.90x）。详见 `experiments/cmp_4way.txt`（含 BASE/ENH/ALT/SEL 历史四方对比原文；BASE/ALT 模型已清理，现保留 ENH 与 SEL）。
...
### 实测对比（2026-07-14，同数据同参数受控 BASE / ENH / ALT / SEL）

> 注：BASE（四关）与 ALT（整体随机交替）模型权重已清理（仅保留 ENH 与 SEL）；下方为历史对比数据，原始生成文本见 `experiments/cmp_4way.txt`。

8000 行语料（merged.txt 前 8000 行，确定性，非随机）× 1 epoch、原 `config_cmp_base.yaml`（四关）/ `config_cmp_enh.yaml`（四开）/ `config_cmp_alt.yaml`（四开 + `enhancement_off_prob: 0.5` 整体随机交替关增强）/ `config_cmp_sel.yaml`（四开 + `enhancement_schedule` 分段**选择性**交替：attn_temp 恒开，qk_norm/residual_gate 按 4 段掩码循环切换）、DML(fp32) 下对照：

- **训练**：BASE(关) **Val 7.8300 / ~3735 tok/s**；ENH(常开) **Val 7.1021 / ~3424 tok/s**；ALT(整体 off=0.5) **Val 7.3543 / ~3596 tok/s**；SEL(分段选择性交替) **Val 7.2254 / ~3413 tok/s**。② 后“常开”开销仅比 BASE 慢约 8%。两种交替都比常开略快（ALT +5%、SEL 持平），但 **Val Loss 均回升**（交替牺牲部分质量）。
- **结论：SEL 优于 ALT**——SEL 的 Val 7.2254 远低于 ALT 的 7.3543、且接近 ENH(7.10)。原因：SEL 让模型**始终保留 attn_temp、且永不完全“全关”**，仅在分段间切换部分增强，比整体 50% 全关更能保住增强收益。故 **ALT 方案已弃用，采用 SEL**。`runtime` 开关已支持按开关粒度（`set_enhancements_active(dict)`），`enhancement_schedule` 即可配置分段选择性交替，多段循环无额外开销。
- **生成（推理时 ALT/SEL 同常开）**：top-k BASE 41.0 / ENH 37.2 / ALT 37.6 / SEL 36.5；IGMCG BASE 13.7 / ENH 10.9 / ALT 10.9 / SEL 10.7 tok/s（增强版约 0.78–0.91x）。
- **文本**：8000 行/1 epoch 仍偏词块破碎；肉眼看 ENH≈SEL > ALT > BASE（SEL 与 ENH 输出片段同样较连贯，ALT 更碎），与 Val 排序一致。
- **鲁棒性探针（ENH vs SEL，控制变量扫温度/top_k）**：`experiments/_robust_enh_sel.py` 对两模型在 `temperature∈{0.5,0.8,1.1,1.4}` 与 `top_k∈{10,30,100}` 下生成并算 self-loss（模型对自身续写的 cross-entropy）。**ENH 在各设置下 self-loss 均略低于 SEL（更自洽/稳健）**，但差距很小（约 0.1–0.3 nat），该训练规模下两者都偏弱；结论：常开 ENH 鲁棒性略优，SEL 接近且更快。
