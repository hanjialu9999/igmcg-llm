...
- **MLA KV 潜空间压缩（use_mla_kv + kv_latent_dim）**：K/V 拼接后下投影到低维潜空间 `c_kv`，cache 只存潜向量；attend 时单次 GEMM 上投影还原 K+V。长序列 KV cache 内存降 `2*dim/kv_latent_dim` 倍。灵感：DeepSeek-V3 MLA。
- **SwiGLU 合并优化（fuse_swiglu）**：将 w1/w3 合并为单个 w13 Linear（输出 2*hidden_dim），前向时 chunk 拆分，减少一次 GEMM 调用。`convert_legacy_state_dict` 自动处理旧 checkpoint 权重转换。默认关向后兼容。
