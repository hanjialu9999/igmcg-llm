...
以下增强均经 `config['model']` 开关控制（默认 `False`），**向后兼容旧权重**，开启后需重新训练才能学到有效参数（门控默认 init 1.0 ≈ 普通残差）：
...
对应配置键：`qk_norm` / `attn_temp` / `residual_gate` / `hybrid_gate`（`models/config_loader.py` 读取）。详见提交 `adfd63d`。
