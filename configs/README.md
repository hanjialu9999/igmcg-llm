...
| `config_dml_full.yaml` · `config_dml_full_b32.yaml` · `config_dml_test.yaml` | AMD DirectML（780M iGPU）训练配置。`gradient_checkpointing` 已关闭（6 层模型显存够用）。SSM/hybrid 现也可在 DML 上训练（选择性扫描已向量化，消除了曾触发 iGPU 设备重置的 kernel 风暴）。 |
