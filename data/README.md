...
| `processed/` | `data_manager.py to-jsonl` 生成的 jsonl（可选）。 | 否（本地） |

> 重新构建语料：`python scripts/data_manager.py merge`（合并 `datasets/`）。词表在训练时由 `data/pretrain_corpus/merged.txt` 自动构建，存于 `checkpoints/vocab.json`。另可用 `merged_sample.txt` 做小样本冒烟。
